// POST /api/login  { user, pass }  -> valida credenciales y crea la cookie de sesión.
...
export async function onRequestPost({ request, env }) {
  ...
  const token = await createToken(
    { user: found.user, scope: found.scope, greystar: !!found.greystar, puede_cargar: !!found.puede_cargar },
    getSecret(env)
  );
  const cookie = [
